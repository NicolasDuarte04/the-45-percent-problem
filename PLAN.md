...
- **cp-14: real Pinnacle ingestion + OSF deviation report for q_market source change.** Hot-fix #102 banners the gap; cp-14 closes it.
- **cp-15: test-fixture hardening pass** — replace pinned literal values that drift with snapshot regeneration (three instances patched in cp-13.5: news_monitor ts, idempotency countCurrent, evaluation_metrics matches_settled). Use computed expectations or tolerances instead.
