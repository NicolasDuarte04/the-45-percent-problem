...
describe("snapshot_meta.json schema", () => {
  ...
  it("matches_settled is a non-negative integer", () => {
    // Relaxed from a `=== 0` pre-tournament pin: the tournament is live and
    // matches settle as it progresses. Cross-artifact consistency with
    // evaluation_metrics (asserted below) is the binding invariant.
    const data = SnapshotMetaSchema.parse(readJson(path.join(LATEST, "snapshot_meta.json")));
    expect(Number.isInteger(data.matches_settled)).toBe(true);
    expect(data.matches_settled).toBeGreaterThanOrEqual(0);
  });
...
describe("divergence.json schema", () => {
  ...
  it("contains priced rows, unless in the pending or stale (no-live-odds) state", () => {
    const data = DivergenceSnapshotSchema.parse(readJson(path.join(LATEST, "divergence.json")));
    if (data.status === "pending" || data.status === "stale") {
      ...
      // than the freshness threshold. Both publish zero rows and stamp no
      // source_book; rows populate once fresh Pinnacle lines land.
...
describe("evaluation_metrics.json schema", () => {
  ...
  it("matches_settled is a non-negative integer", () => {
    // Relaxed from `=== 0`: matches_settled is now sourced from the live
    // match_outcomes count (same as snapshot_meta), so it tracks the
    // tournament. Cross-artifact consistency is the real invariant (below).
...
describe("cross-artifact consistency", () => {
  ...
  it("team fifa_codes in tournament.json exactly match files in teams/ (bidirectional, no duplicates)", () => {
    // cp-12: tightened from a one-directional subset check (tournament
    // codes ⊆ teams/ files) to bidirectional set-equality + no-duplicate.
    // The old check passed the live corruption that shipped 2026-05-12 —
    // Congo DR (COD) listed twice and Tunisia (TUN) dropped — because every
    // tournament code (incl. both CODs) still had a teams/ file, and TUN
    // being present-but-unreferenced was never tested.
    const teamsDir = path.join(LATEST, "teams");
    const teamFiles = new Set(
      fs.readdirSync(teamsDir).filter((f) => f.endsWith(".json")).map((f) => f.replace(".json", ""))
    );
    const tournament = TournamentSnapshotSchema.parse(readJson(path.join(LATEST, "tournament.json")));
    const codes = tournament.teams.map((t) => t.fifa_code);
    const codeSet = new Set(codes);

    // (a) no duplicate fifa_codes in tournament.json (catches COD-twice).
    const dupes = [...new Set(codes.filter((c, i) => codes.indexOf(c) !== i))];
    expect(dupes, `duplicate fifa_codes in tournament.json: ${dupes.join(", ")}`).toEqual([]);

    // (b) bidirectional set equality: every tournament code has a teams/
    //     file AND every teams/ file is in tournament.json (catches the
    //     dropped-Tunisia case the old subset check missed).
    const missingFiles = codes.filter((c) => !teamFiles.has(c));
    expect(missingFiles, `teams/ file missing for: ${missingFiles.join(", ")}`).toEqual([]);
    const missingFromTournament = [...teamFiles].filter((c) => !codeSet.has(c));
    expect(missingFromTournament, `tournament.json missing teams present in teams/: ${missingFromTournament.join(", ")}`).toEqual([]);
  });
  ...
  it("every match_id referenced in divergence.json has a backing detail file", () => {
    // Group rows (round GRP, M{NN}) render at /match/[id] and must have a
    // matches/{id}.json. cp-30: knockout rows (KO-FD ids) render at the sibling
    // /match/live/[id] route and must instead have a matches_live/{id}.json.
    // A row pointing at the wrong (or missing) file 404s at runtime; see commit
    // 0b9db6a for the [:10]-slice incident this test guards against.
    const matchFiles = new Set(
      fs.readdirSync(path.join(LATEST, "matches"))
    ...
    expect(
      [...new Set(missing)],
      `divergence rows missing a backing detail file: ${[...new Set(missing)].join(", ")}`,
    ).toEqual([]);
  });
