...
**Status:** Not started. Architectural decision Q2 already made (revalidate-on-write).
**Scope:** Wire `revalidatePath("/bracket")` into the admin endpoint at `website/src/app/api/admin/match-outcomes/route.ts` after a successful upsert. Also wire it into the nightly cron's final step so the live site picks up fresh snapshots without an explicit redeploy.
**Acceptance:** Manually entering a settled outcome via `/api/admin/match-outcomes` produces a visible change on `/bracket` within 10 minutes. The cron run logs include the revalidation call.
**Effort:** Half to one day, including the static-vs-dynamic decision on the bracket route (the existing `force-static` is intended to be preserved per Q2; revalidate-on-write is compatible with that).
**Prompt:** To be written after cp-12 ships.
