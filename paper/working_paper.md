...
- The champion model M★ is selected from the candidate set $\{M_0, M_1, M_2, M_3\}$ by a two-condition gate (lowest cross-validation log-loss, conditional on a pre-registered standard-error margin against M0). Selection uses only pre-tournament data and is completed before the opening match. As executed, only the first condition binds and the margin condition is recorded rather than enforced; see Section 6.3.1.
...
- The kill criterion is sealed: a two-condition gate on cross-validation log-loss against M0, with the standard-error threshold sealed in `pre_reg_constants.yaml::kill_criterion.threshold_standard_errors` at 2.0. The pre-registered consequence on firing is `pivot_paper_framing` — a change to what the paper claims, not an automatic demotion of M★. The registration seals the threshold and the consequence; it does not seal which standard-error construction the threshold is measured against. The only construction implemented against that constant, `evaluation/accuracy_metrics.check_kill_criterion`, is a paired difference: $d_i = \ell^{M\star}_i - \ell^{M_0}_i$ scored per match, with $\mathrm{se} = \mathrm{sd}(d, \mathrm{ddof}=1)/\sqrt{n}$. Section 6.3.1 records where the project has published figures that are not that construction.
...
Two cross-validation batteries were executed, and the distinction matters for reading Sections 6.3 and 6.4, so it is drawn here rather than left implicit.

The **Phase 8 pre-registration battery** (`src/calibration/run_cv_battery.py`, artifact `evaluation/cv_battery_result.json`) is the scheme this section describes: a stratified five-fold partition of the 283 calibration matches from 2010 to 2021, with the 64 2022 World Cup matches held out entirely and scored separately as the final exam. The metric is per-match log-loss, averaged across folds, with the standard error of the cross-fold mean reported alongside. This is the battery that computes and records the standard-error gate.

The **Phase 4 model-development battery** (`models/model_registry.py`, artifact `data/calibration/cv_battery_results.json`) is a different scheme: five fixed tournament windows rather than a stratified partition, one of which is empty in this corpus and one of which is the 2022 hold-out itself. It is the source of the adjudication table in Section 6.3 and of the calibrated $\tau^{*}$, $w^{*}$ and $\theta$. Section 6.3.1 sets out how the two batteries differ numerically, and Section 6.4 sets out what the Phase 4 fold construction costs the hold-out claim.

The pre-registered decision rule is a two-condition gate, not a single-criterion ranking. M★ is the candidate with the lowest mean CV log-loss, conditional on the pre-registered standard-error margin against M0. Both conditions were intended to hold, with failure on either firing the kill criterion, and the bar is sealed in the OSF registration before any 2026 prediction is made. **In the code as executed, only the first condition binds.** The margin condition is computed, recorded and reported, but nothing downstream consumes its result; Section 6.3.1 documents the gap between the described rule and the implemented one, and Section 6.3 reports what the gate actually returned.
...
M2_fifa cleared the first condition — it had the lowest mean CV log-loss of the candidate set — and was locked as M★ (`data/calibration/champion_model.json::CHAMPION_LOCKED: true`). **It did not clear the second condition.** The pre-registration battery recorded the failure explicitly: `evaluation/cv_battery_result.json::champion_selection` carries `"sanity_gate_passed": false` and a `decision_narrative` reading "WARNING: sanity gate NOT passed — M★ only 1.75 SE better than M0 (threshold ≥ 2.0 SE)."

The champion was nevertheless retained, for two reasons that must be stated together. The first is procedural and defensible: the pre-registered consequence of a failed gate is `pre_reg_constants.yaml::kill.action = pivot_paper_framing`, not demotion of M★. The gate was designed to change what the paper claims, not which model runs. The second is an implementation gap and is not defensible as a design choice: the second condition is not binding anywhere in the code. `src/calibration/run_cv_battery.py:623` emits a `log.warning` on failure and execution continues; the champion identity was fixed earlier in the same function and is never revisited. `models/model_registry.py:849` implements eligibility as `scores[mid] < l_m0` alone, with no standard-error margin. Section 6.3.1 records this discrepancy between the described gate and the implemented one.

M3_macro improved over M0 in point estimate but the improvement was small enough that the additional parametric structure does not earn its keep at this corpus size. M0_elo serves as the baseline. M1_form was disqualified for being statistically worse than M0.

## 6.3.1 What the Two Standard-Error Readings Actually Measure

Two standard-error readings of the same M2-versus-M0 gap have been published by this project, 6.22 SE and 1.75 SE, and both have been described in earlier project documents as a "marginal" and a "paired-difference" reading of one number. That description is wrong, and this section corrects it. Neither figure is a paired difference. Both divide the gap by a dispersion measure of M2 alone; neither uses M0's variance, and neither uses the fold-level pairing.

The two figures also come from two different cross-validation batteries, which is the larger part of why they diverge:

- **6.22** comes from `data/calibration/cv_battery_results.json`, the Phase 4 model-development battery (`models/model_registry.py`), whose folds are the five fixed tournament windows of `_CV_FOLDS`. It is $0.040960 / 0.006587$, where the denominator is `sigma_CV`, computed at `models/model_registry.py:822` as `np.std(valid_folds, ddof=1)` — the standard deviation *between* M2's fold losses, with no division by $\sqrt{n}$. It is a gap expressed in units of the champion's between-fold spread.
- **1.75** comes from `evaluation/cv_battery_result.json`, the Phase 8 pre-registration battery (`src/calibration/run_cv_battery.py`), whose folds are a stratified five-fold partition of the calibration matches only. It is $0.031685 / 0.018087$, where the denominator is M2's `se_log_loss`, the standard error of M2's cross-fold mean ($\mathrm{sd}/\sqrt{5}$). It is a gap expressed in units of the champion's mean standard error. The gate code makes this explicit: `se_for_gate = champ_se` at `src/calibration/run_cv_battery.py:618` is the champion's own SE, not an SE of a difference.

Written out on the same grid, with $d_i = \mathrm{LL}^{M2}_i - \mathrm{LL}^{M0}_i$ over the folds of each battery and $\mathrm{SE}_{\text{paired}} = \mathrm{sd}(d, \mathrm{ddof}=1)/\sqrt{n}$:

| Battery | $n$ | gap | gap / between-fold SD | gap / mean SE | paired reading |
|---|---|---|---|---|---|
...
Three things follow. First, the published pair 6.22 and 1.75 is not a marginal-versus-paired contrast; it is a between-fold-SD reading of one battery set beside a mean-SE reading of a different battery. Second, most of the distance between them is neither the $\sqrt{n}$ nor the pairing but the fold construction: M2's between-fold spread is 0.006587 in the Phase 4 tournament folds and 0.040443 in the Phase 8 stratified folds, a factor of 6.1, because tournament-window folds make M2's fold losses far more homogeneous than a stratified partition does. Third, the genuinely paired statistic — the construction the pre-registered kill criterion is actually implemented on, at `evaluation/accuracy_metrics.check_kill_criterion` — is 1.96 on the Phase 4 folds and 2.24 on the Phase 8 folds. It straddles the 2.0 bar. The honest summary is that this gap sits at the threshold: not comfortably above it, as 6.22 suggests, and not clearly below it, as 1.75 suggests.

This paper therefore reports the gap under names that describe the arithmetic. 6.22 is the gap in units of the champion's **between-fold standard deviation**. 1.75 is the gap in units of the standard error of the champion's **cross-fold mean**. The term *paired-difference SE* is reserved for the construction that is one, and is used here only for the 1.96 and 2.24 figures above and for the live Round-of-16 checkpoint, which computes a paired per-match SE.

**The gate is described as binding and is not implemented as binding.** Sections 3 and 6.2 describe M★ selection as a two-condition gate on which "both conditions must hold" and "failure on either fires the kill criterion". The code implements one condition. `models/model_registry.py:849` selects among models satisfying `scores[mid] < l_m0` — lowest CV log-loss, no margin term. `src/calibration/run_cv_battery.py:613-644` computes `sanity_gate_passed` and, on failure, writes a warning to the log, a sentence into `decision_narrative`, a row into the PDF report and a boolean into the JSON artifact. It does not raise, does not reselect, and does not alter `champion_id`, which was assigned before the gate ran. No other code path makes the second condition binding. What the second condition does bind is the paper's framing, via `kill.action: pivot_paper_framing` — which is what this section and the corrected Section 6.3 carry out.
...
The hold-out ranking matches the cross-validation ranking, with M2 winning, M3 second, M0 third, M1 last and disqualified. The absolute hold-out values track the cross-validation values to better than 0.05 log-loss on every model.

**This agreement is not independent evidence, and an earlier version of this section described it as though it were.** The four hold-out log-losses in the table above are numerically identical, to six decimal places, to the fifth entry of each model's `fold_losses` array in `data/calibration/cv_battery_results.json` — the same file the Section 6.3 adjudication table is drawn from. They agree because they are the same 64 matches scored under the same locked parameters, and because fold 5 of the Phase 4 battery *is* the hold-out. `models/model_registry.py:53-59` defines the Phase 4 folds as five fixed tournament windows, the fifth of which is `("2021-12-31", "2022-01-01", "2022-12-18") # fold 5: 2022 WC`. Fold 5's evaluation set is selected on the date window alone, with the holdout deliberately admitted — the comment at `models/model_registry.py:788` reads "Eval uses all matches in the window (holdout allowed in eval)". The 2022 window contains exactly the 64 hold-out matches and nothing else, because the corpus has no other 2022 fixtures. `L_CV` is then the mean over the non-empty folds at `models/model_registry.py:820-821`, so the 2022 hold-out contributes one quarter of the statistic that selected M★.

Two consequences follow, and they are of different severity.

The weaker one concerns the loss values themselves. Every fold trains on `(~matches["is_holdout"])` (`models/model_registry.py:787`), so no model was ever fitted on a 2022 result. The 0.987659 reported for M2 is a genuine out-of-sample loss in the prediction sense.

The stronger one concerns model selection, and it is the claim this section previously overstated. The champion was chosen by `L_CV`, and `L_CV` contains the hold-out. So was the M2 blend weight: `w^{*}` is the argmin of a grid search whose per-w CV mean also averages over fold 5 (`models/model_registry.py:710-734`). Selection therefore saw the hold-out. Section 6.4 cannot be read as a second, independent surface confirming a decision made without it; it is one of the four folds that made the decision, reported a second time. This also contradicts the project's own standing rule, recorded in `CLAUDE.md`, that the 2022 hold-out "must never be used during calibration".

What can be said is narrower, and is worth saying because it is checkable. Recomputing the Phase 4 means with fold 5 excluded leaves the ranking unchanged — M2 0.995274, M3 1.036778, M0 1.039726, M1 1.074386 — so the leakage did not manufacture the champion. It did inflate the appearance of independent corroboration, and it moves the M2-versus-M0 gap from −0.040960 to −0.044452. A genuinely independent 2022 reading does exist: the Phase 8 battery at `src/calibration/run_cv_battery.py` partitions the 283 calibration matches into stratified folds and scores the 64 hold-out matches separately, reporting `holdout_log_loss` for M2 of 0.987659. That is the number this section should have cited as the independent surface, and it is the same number only because both paths score the same 64 matches with the same locked parameters.

**The Phase 4 battery ran on four folds, not five.** Fold 2 of `_CV_FOLDS` covers 2016-01-01 to 2016-07-10, and the corpus contains no 2016 tournament (`ingestion/fetch_historical_matches.py:68-83` lists World Cups 2010, 2014, 2018, 2022 and six continental tournaments, none of them in 2016). The fold is empty, `models/model_registry.py:794-796` appends `NaN`, and `export_cv_report` serialises it as `null` — the second element of every `fold_losses` array in the sealed artifact. Every Phase 4 mean, standard deviation and delta in Section 6.3 is therefore over $n = 4$, not the five the methodology describes. The Phase 8 battery is unaffected: its five stratified folds are all populated.
...
Three limitations of the adjudication itself are recorded in Section 6 rather than here, because they are properties of the executed protocol rather than of its scope, and a reader should meet them where the result is reported. The model-development cross-validation ran on four folds, not the five its scheme specifies, and one of those four is the 2022 hold-out, so the hold-out did not function as an independent check on selection (Section 6.4). The standard-error margin the protocol describes as a binding second condition is computed and logged but never consumed, so selection ran on log-loss alone (Section 6.3.1). And the two standard-error figures this project has published, 6.22 and 1.75, are neither of them the paired construction the kill criterion is implemented on; on that construction the M2-versus-M0 gap sits at 1.96 to 2.24, on the 2.0 bar rather than clear of it (Section 6.3.1). These are reporting corrections, filed as amendment v1.2; no sealed statistic moved.
